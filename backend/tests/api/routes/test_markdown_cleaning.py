...
def test_create_returns_202_and_dispatches_once(
    ...
    client, _engine, _user, dispatcher, payload, _request_session_ids, _closed_session_ids = (
        api_context
    )
...
def test_db_dependency_is_request_scoped_and_closed(
    ...
    client, engine, _user, _dispatcher, payload, request_session_ids, closed_session_ids = (
        api_context
    )
    ...
    assert client.get(f"/api/v1/markdown-cleaning/tasks/{task_id}").json()[
        "status"
    ] == "running"
...
def test_create_rejects_changed_request_for_same_idempotency_key(
    ...
    client, _engine, _user, _dispatcher, payload, _request_session_ids, _closed_session_ids = (
        api_context
    )
    assert (
...
def test_get_returns_snapshot_for_pending_and_running_statuses(
    ...
    client, engine, _user, _dispatcher, payload, _request_session_ids, _closed_session_ids = (
        api_context
    )
